...
void BJ_MD5Update(BJ_MD5_CTX *context,unsigned char *input,unsigned int inputlen)
{
    ...
    if(inputlen >= partlen)
    {
        ...
        BJ_MD5Transform(context->state,context->buffer);
        for(i = partlen;i+64 <= inputlen;i+=64)
            BJ_MD5Transform(context->state,&input[i]);
        index = 0;
    }
    else
    {
...
void BJ_MD5Final(BJ_MD5_CTX *context,unsigned char digest[16])
{
    ...
    BJ_MD5Encode(bits,context->count,8);
    BJ_MD5Update(context,PADDING,padlen);
    BJ_MD5Update(context,bits,8);
    BJ_MD5Encode(digest,context->state,16);
}
...
void BJ_md5_encode_str(char *result, unsigned char *src)
{
    ...
    BJ_MD5_CTX md5;
    BJ_MD5Init(&md5);
    BJ_MD5Update(&md5, src, (unsigned int)strlen(src));
    BJ_MD5Final(&md5, digest);
    for (int i = 0; i < 16; ++ i)
    {
        sprintf(result, "%02x", (unsigned char)digest[i]);
        result += 2;
    }
